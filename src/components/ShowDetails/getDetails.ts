...
import Logo from '../../assets/ongaku-bg.svg';
...
export class Details {
  ...
  static async parse(route: string, user: User, type: string) {
    let moreInfo = await getFromDiscogs(
      `/${route}`,
      user.token,
      user.tokenSecret
    );
    if (type === 'masters' && moreInfo.main_release_url) {
      moreInfo = await getFromDiscogs(
        `/${moreInfo.main_release_url.split('.com/')[1]}`,
        user.token,
        user.tokenSecret
      );
    }
    ...
    if (type === 'releases' || type === 'masters') {
      const wantlist = await getFromDiscogs(
        `/users/${user.username}/wants?per_page=100`,
        user.token,
        user.tokenSecret
      );
      const collection = await getFromDiscogs(
        `/users/${user.username}/collection/${
          moreInfo.resource_url.split('.com/')[1]
        }`,
        user.token,
        user.tokenSecret
      );
      userData = {
        user_data: {
          in_wantlist: !!wantlist.wants.find(
            (release: { id: number }) => release.id === moreInfo.id
          ),
    ...
    if (type === 'artists' || type === 'labels') {
      ...
      const releases = await getFromDiscogs(
        `/${moreInfo.releases_url.split('.com/')[1]}`,
        user.token,
        user.tokenSecret
      );
