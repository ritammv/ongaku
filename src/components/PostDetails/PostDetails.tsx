...
const Postdetails: React.FC<Props> = ({ postId }) => {
  ...
  function postComment(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    apiclient.createComment(post.id, user.id, commentBody)
      .then((newComment) => {
  ...
  return (
    ...
    >
      ...
      <form className='comment_form' onSubmit={postComment}>
        ...
        <Textarea onChange={e => handleChange(e, 'comment')} name='comment' placeholder="Share your thoughts" />
